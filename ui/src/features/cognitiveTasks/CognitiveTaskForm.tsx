...
import type { CognitiveTask, TriggerKind } from './cognitiveTaskTypes';

export type CognitiveTaskDraft = Omit<CognitiveTask, 'id'> & { id?: string };

function ensureTriggerShape(trigger: CognitiveTaskDraft['trigger']): CognitiveTaskDraft['trigger'] {
...
export function CognitiveTaskForm(props: {
  value: CognitiveTaskDraft;
  onChange: (next: CognitiveTaskDraft) => void;
  readOnly: boolean;
}): React.JSX.Element {
  const v = props.value;
  const trigger = ensureTriggerShape(v.trigger);

  function set<K extends keyof CognitiveTaskDraft>(key: K, val: CognitiveTaskDraft[K]): void {
  ...
  return (
    <Box display="flex" flexDirection="column" gap={2}>
      ...
      <FormControl fullWidth disabled={props.readOnly}>
        <InputLabel id="cognitive-task-category">Category</InputLabel>
        <Select
          labelId="cognitive-task-category"
          label="Category"
          value={v.category}
          onChange={(e) => set('category', String(e.target.value) as CognitiveTask['category'])}
        >
      ...
      <FormControl fullWidth disabled={props.readOnly}>
        <InputLabel id="cognitive-task-trigger-kind">Trigger</InputLabel>
        <Select
          labelId="cognitive-task-trigger-kind"
          label="Trigger"
          value={trigger.kind}
