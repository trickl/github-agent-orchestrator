...
async function waitForRulesToLoad(): Promise<void> {
  await screen.findByText('Complexity review');
}

test('Rules list renders and filter works', async () => {
  const user = userEvent.setup();
  renderWithAppProviders(<RulesPage />, { route: '/rules' });
  await waitForRulesToLoad();
...
test('Edit rule flow: open editor, change name, save updates list', async () => {
  const user = userEvent.setup();
  renderWithAppProviders(<RulesPage />, { route: '/rules' });
  await waitForRulesToLoad();
...
test('Run now flow: confirm then snackbar shown', async () => {
  ...
  await screen.findByText('Run rule now?');
