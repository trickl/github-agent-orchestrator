...
test('Docs page renders goal, target, and current state', async ({ page }) => {
  ...
  await expect(page.getByText('.agent-orchestrator/state/current_state.md')).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Current State', level: 1 })).toBeVisible();
});
