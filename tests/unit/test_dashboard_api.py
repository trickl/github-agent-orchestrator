...
def test_dashboard_health_and_docs(monkeypatch, tmp_path: Path) -> None:
    ...
    assert any(r.get("id") == "review-complexity.md" for r in rules)


def test_rules_create_run_creates_queue_item(monkeypatch, tmp_path: Path) -> None:
    planning = tmp_path / "planning"
    agent_state = tmp_path / "agent_state"
    ...
    run = client.post(f"/api/rules/{created['id']}/run").json()
